/*!
ECDSA(Elliptic curve digital sign algorithm) have 3 steps:
1. Key Generation

y^2 = x^3 + ax + b mod p
|G| = q (prime order)
select random d: 0<d<q
B = d*A
Public key is B, and private key is d

2.Message sign
 a. Choose Random K, 0<k<q
 b.R = K*A,  r=x
...
impl ECDSA {
    ...
    pub fn generate_key_pair(&self) -> Result<(BigUint, FiniteFieldElement), ECDSAErrors> {
    ...
    pub fn generate_pub_key(&self, priv_key: &BigUint) -> Result<FiniteFieldElement, ECDSAErrors> {
        self.elliptic_curve
    ...
    pub fn sign(
        ...
        if let FiniteFieldElement::Coordinate(r, _) = r_point {
            let s = GaloisField::mult(&r, priv_key, &self.q_order).map_err(|_| {
                ECDSAErrors::OperationFailure("Error multiplying r * priv_key".into())
            })?;

            let s = GaloisField::add(&s, hash, &self.q_order).map_err(|_| {
                ECDSAErrors::OperationFailure("Error adding hash + r * priv_key".into())
            })?;

            let k_inv = GaloisField::inv_mult_prime(k_random, &self.q_order)
                .map_err(|_| ECDSAErrors::OperationFailure("Error computing k_inv".into()))?;

            let s = GaloisField::mult(&s, &k_inv, &self.q_order).map_err(|_| {
    ...
    pub fn verify(
        ...
        let s_inv = GaloisField::inv_mult_prime(s, &self.q_order)
            .map_err(|_| ECDSAErrors::OperationFailure("Error computing s_inv".into()))?;

        let u1 = GaloisField::mult(&s_inv, hash, &self.q_order).map_err(|_| {
            ECDSAErrors::OperationFailure("Error multiplying s_inv and hash".into())
        })?;

        let u2 = GaloisField::mult(&s_inv, r, &self.q_order)
        ...
        if let FiniteFieldElement::Coordinate(xp, _) = p {
            return Ok(xp == *r);
        }
...
mod test {
    ...
    fn test_sign_verify() {
        ...
        let a_gen = FiniteFieldElement::Coordinate(BigUint::from(5u32), BigUint::from(1u32));
    ...
    fn test_secp256_sign_verify() {
        ...
        let a_gen = FiniteFieldElement::Coordinate(gx, gy);
